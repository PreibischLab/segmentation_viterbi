function [Xs, Ys] = addNormThenViterbi(img, nucContour, lambda, normLength, m)
...
%      figure;imshow(img,[]);hold on;plot(Ys, Xs, 'b--*');
%     plot(YsInner,XsInner,'--*');
%     plot(YsOuter,XsOuter,'--*');
...
%     plot(Ys1, Xs1, 'g--*'); plot(Ys2, Xs2, 'b--*');plot(Ys1(1), Xs1(1), 'r--*');plot(Ys2(1), Xs2(1), 'y--*');
%     plot(Ys, Xs, 'm--*');
